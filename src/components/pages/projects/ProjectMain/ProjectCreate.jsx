...
function ProjectCreate({ open, handleClose,refetch }) {
  ...
  const [getSubSubActivities, { data: subSubActivitiesData, isLoading: subSubActivitiesLoading }] =         useGetMultipleSubSubActivitiesMutation();
  ...
  const [selectTotalLandArea,setSelectTotalLandArea] = useState();
    const landOptions = landBankData?.data?.map(land => ({
      id: land.id,
    ...
    const handleLandChange = (event, value) => {
      ...
      const handleTotalAreaChange = (e) => {
        ...
        if (isNaN(numericValue) || numericValue < 0) {
            ...
            setAreaError(`Selected area cannot exceed available land area of ${availableArea}`);
        } else {
            setAreaError('');
        }
    ...
    const { data: subActivitiesData, isLoading: subActivitiesLoading } = useGetDropdownSubActivitiesQuery(activity, {
        skip: !activity,
    });

    const subActivitiesOptions = subActivitiesData?.data[0]?.sub_activity?.map((sub) => ({
    ...
    return (
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
            <DialogContent>
                <h2 className="text-[#29346B] text-2xl font-semibold mb-5">Create Project</h2>
                    ...
                    <div className='flex flex-col gap-2'>
                        <label className="block mb-1 text-[#29346B] text-lg font-semibold">
                            ...
                            <Autocomplete
                                ...
                                value={companyOptions.find(comp => comp.id === companyName) || null} // Match selected value
                                onChange={(_, value) => setCompanyName(value?.id || '')} // Store only ID in state
                                renderInput={(params) => (
                                    <TextField {...params} fullWidth placeholder="Select Company" sx={inputStyles} />
                                )}
                            />
                    </div>
                    <div className='flex flex-col gap-2'>
    ...
    <div className='flex flex-col gap-2'>
        ...
        />
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} fullWidth placeholder="Select CI/Utility" sx={inputStyles} />
                        )}
                    />
                    </div>
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} fullWidth placeholder="Select CPP/IPP" sx={inputStyles} />
                        )}
                    />
                    </div>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField {...params} fullWidth placeholder="Select Electricity Line" sx={inputStyles} />
                            )}
                        />
                    </div>
            ...
            getOptionLabel={(option) => option.label}
            value={subActivitiesOptions.filter(sub => subActivities.includes(sub.id)) || []}
            onChange={(_, selected) => setSubActivities(selected?.map(sub => sub.id) || [])}
            disabled={!activity}
            loading={subActivitiesLoading}
            isOptionEqualToValue={(option, value) => option?.id === value?.id} // Ensures correct selection
            renderInput={(params) => (
                <TextField {...params} fullWidth placeholder="Select Sub Activity" sx={inputStyles} />
            )}
        />
        </div>
    ...
    /> */}
        ...
        isOptionEqualToValue={(option, value) => {
            if (typeof value === 'object' && value !== null && typeof option === 'object' && option !== null) {
    ...
    />
            ...
            <div className='flex flex-col gap-2'>
                ...
                <div className='flex flex-row justify-center my-4'>
                    <Button onClick={handleSubmit} variant="contained" style={{ backgroundColor: '#FF8C00', color: 'white', fontWeight: 'bold', padding: '10px 30px', fontSize: '16px', textTransform: 'none' }}>
                        Submit
                    </Button>
                </div>
            </DialogContent>
