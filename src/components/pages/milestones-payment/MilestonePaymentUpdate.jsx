...
import { useUpdateMilestonePaymentMutation } from '../../../api/milestonePayment/milestonePaymentApi';

// Edit Payment Modal Component
function EditPaymentModal({ open, handleClose, paymentData, refetch }) {
  const [updateMilestonePayment, { isLoading }] = useUpdateMilestonePaymentMutation();
  ...
  useEffect(() => {
    if (paymentData) {
      // Extract percentage from gst_amount string if it contains '%'
      let gstPercentage = "";
      if (paymentData.gst_amount && paymentData.gst_amount.includes('%')) {
        gstPercentage = paymentData.gst_amount.replace('%', '');
      }
      ...
      setFormData({
        milestone_id: paymentData.milestone,
        party_name: paymentData.party_name || "",
        po_number: paymentData.po_number || "",
        invoice_number: paymentData.invoice_number || "",
        total_amount: paymentData.total_amount || "",
        gst_percentage: gstPercentage,
        gst_amount: paymentData.gst_amount || "",
        paid_amount: paymentData.paid_amount || "",
        pending_amount: paymentData.pending_amount || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateMilestonePayment({
        id: paymentData.id,
        ...formData
      }).unwrap();
