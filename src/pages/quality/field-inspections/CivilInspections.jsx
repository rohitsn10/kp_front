...
import { 
  useGetCivilRfiQuery, // Using Civil RFI query hook
...
function CivilInspections() {
  ...
  // Fetch civil RFI data from API using the civil-specific hook
  const { 
    data: civilRfiResponse, 
    isLoading: isLoadingRfi, 
    error ,
    refetch
  } = useGetCivilRfiQuery(projectId);
  ...
  useEffect(() => {
    if (!civilRfiResponse) return;
    
    setIsLoading(true);
    
    // Short delay for better UX
    setTimeout(() => {
      let result = [...civilRfiResponse.data];
  ...
  }, [searchTerm, sortOption, civilRfiResponse, itemsPerPage]);
  ...
  return (
    <div className="min-h-screen p-4 bg-white m-1 md:m-8 rounded-md">
      <h2 className="text-2xl font-semibold text-[#29346B] text-center mb-4">Civil Inspections</h2>
      ...
      <div className="mx-auto mt-6">
        ...
        ) : civilRfiResponse && paginatedItems.length ? (
          <div className="overflow-x-auto">
            <table id="civil-rfi-table" className="min-w-full bg-white border border-gray-300">
              <thead>
            ...
            <div className="text-sm text-gray-600 mt-2 text-center">
              ...
              {filteredItems.length !== (civilRfiResponse?.data?.length || 0) && 
                ` (filtered from ${civilRfiResponse?.data?.length || 0} total)`}
            </div>
          </div>
        ) : (
          renderEmptyState()
      ...
      <CreateRfiForm 
        ...
        projectId={projectId || ''}
        category="civil" // Changed to "civil"
        onSuccess={()=>refetch()}
      />
      <RfiOutcomeForm 
...
export default CivilInspections;