...
import { 
  useGetElectricalRfiQuery, // Changed from useGetMechanicalRfiQuery to useGetElectricalRfiQuery
...
function ElectricalInspections() {
  const { projectId } = useParams();
  const [searchTerm, setSearchTerm] = useState("");
  const [sortOption, setSortOption] = useState("all");
  const [filteredItems, setFilteredItems] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [openRfiForm, setOpenRfiForm] = useState(false);
  ...
  // Fetch electrical RFI data from API using the electrical-specific hook
  const { 
    data: electricalRfiResponse, 
    isLoading: isLoadingRfi, 
    error,
    refetch
  } = useGetElectricalRfiQuery(projectId);
  ...
  useEffect(() => {
    if (!electricalRfiResponse) return;
    
    setIsLoading(true);
    
    // Short delay for better UX
    setTimeout(() => {
      let result = [...electricalRfiResponse.data];
  ...
  }, [searchTerm, sortOption, electricalRfiResponse, itemsPerPage]);
  ...
  return (
    <div className="min-h-screen p-4 bg-white m-1 md:m-8 rounded-md">
      <h2 className="text-2xl font-semibold text-[#29346B] text-center mb-4">Electrical Inspections</h2>
      ...
      <div className="mx-auto mt-6">
        ...
        ) : electricalRfiResponse && paginatedItems.length ? (
          <div className="overflow-x-auto">
            <table id="electrical-rfi-table" className="min-w-full bg-white border border-gray-300">
              <thead>
            ...
            <div className="text-sm text-gray-600 mt-2 text-center">
              ...
              {filteredItems.length !== (electricalRfiResponse?.data?.length || 0) && 
                ` (filtered from ${electricalRfiResponse?.data?.length || 0} total)`}
            </div>
          </div>
        ) : (
          renderEmptyState()
      ...
      <CreateRfiForm 
        ...
        projectId={projectId || ''}
        category="electrical" // Changed from "mechanical" to "electrical"
        onSuccess={()=>refetch()}
      />
      <RfiOutcomeForm 
